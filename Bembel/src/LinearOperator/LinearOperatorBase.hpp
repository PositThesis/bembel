...
struct LinearOperatorBase {
  ...
  Eigen::Matrix<
      ...
      getFunctionSpaceVectorDimension<LinearOperatorTraits<Derived>::Form>() *
          LinearOperatorTraits<Derived>::NumberOfFMMComponents>
  evaluateFMMInterpolation(const SurfacePoint<ptScalar> &p1,
                           const SurfacePoint<ptScalar> &p2) const {
  ...
  int getNearfieldQuadratureDegree(int ansatz_degree, ptScalar distance,
    ...
    ptScalar numerator =
        ...
        (2 - ansatz_degree + LinearOperatorTraits<Derived>::OperatorOrder) *
            distance_log;
    ptScalar denominator = (level + 2) * log(2.) + distance_log;

    return int(0.5 * numerator / denominator);
  }
