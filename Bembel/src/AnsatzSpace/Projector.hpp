...
struct projector_matrixmaker_ {
  static Eigen::SparseMatrix<ptScalar> makeMatrix(
      const Bembel::SuperSpace<Derived, ptScalar>& super_space,
      const int knotrepetition) {
    assert(false && "This needs to be specialized");
    return Eigen::SparseMatrix<ptScalar>(1, 1);
  }
};
}  // namespace ProjectorRoutines
...
class Projector {
 public:
  ...
  //////////////////////////////////////////////////////////////////////////////
  Projector() {}
  Projector(const SuperSpace<Derived, ptScalar>& super_space, const int knot_repetition) {
    init_Projector(super_space, knot_repetition);
  }
  //////////////////////////////////////////////////////////////////////////////
  /// init
  //////////////////////////////////////////////////////////////////////////////
  void init_Projector(const SuperSpace<Derived, ptScalar>& super_space,
  ...
  const Eigen::SparseMatrix<ptScalar>& get_projection_matrix() {
    return projector_;
  }
  ...
  //////////////////////////////////////////////////////////////////////////////
 private:
  Eigen::SparseMatrix<ptScalar> makeProjectionMatrix(
      const SuperSpace<Derived, ptScalar>& super_space, const int knotrepetition) {
    return ProjectorRoutines::projector_matrixmaker_<
        Derived,
        LinearOperatorTraits<Derived>::Form, ptScalar>::makeMatrix(super_space,
                                                         knotrepetition);
  }
  ...
  Eigen::SparseMatrix<ptScalar> projector_;
  Projector(const Projector<Derived, ptScalar>& other);
  Projector(Projector<Derived, ptScalar>&& other);
  Projector& operator=(const Projector<Derived, ptScalar>& other);
  Projector& operator=(Projector<Derived, ptScalar>&& other);
  //////////////////////////////////////////////////////////////////////////////
};
...
template <typename Derived, typename ptScalar>
inline _proj_info<ptScalar> makeLocalProjectionTriplets(
    const SuperSpace<Derived, ptScalar>& super_space, const int pp1x, const int pp1y,
  ...
  // interior knots.
  std::vector<ptScalar> c_space_knot_x =
      MakeUniformKnotVector<ptScalar>(pp1x, n - 1, knotrepetition);
  std::vector<ptScalar> c_space_knot_y =
      MakeUniformKnotVector<ptScalar>(pp1y, n - 1, knotrepetition);
  ...
  std::vector<ptScalar> mask = MakeInterpolationMask<ptScalar>(maximal_polynomial_degree);
  ...
  Eigen::Matrix<ptScalar, -1, -1> system(
      masksize * masksize,
  ...
  {
    ptScalar vals_y[Constants::MaxP + 1];
    ptScalar vals_x[Constants::MaxP + 1];
    for (int iy = 0; iy < maximal_polynomial_degree; ++iy) {
      Bembel::Basis::ShapeFunctionHandler<ptScalar>::evalBasis(
  ...
  const Eigen::PartialPivLU<Eigen::Matrix<ptScalar, -1, -1>> pplu(system);
  ...
  for (auto element = super_space.get_mesh().get_element_tree().cpbegin();
       ...
       ++element) {
    // s = x
    const ptScalar pos_x = element->llc_(0);
    const ptScalar pos_y = element->llc_(1);

    const ptScalar h = element->get_h();
    const ptScalar mid_x = pos_x + (.5 * h);
    const ptScalar mid_y = pos_y + (.5 * h);
    ...
    std::vector<int> nonzero_dofs_x;
    std::vector<int> nonzero_dofs_y;
    ...
    for (int dof_y : nonzero_dofs_y) {
      for (int dof_x : nonzero_dofs_x) {
        // The next few lines are to check if the element is within the support
        // of the basis function given by dof_x and dof_y.
        std::vector<ptScalar> local_mask_x(masksize);
        std::vector<ptScalar> local_mask_y(masksize);
...
template <typename Derived, typename ptScalar>
struct projector_matrixmaker_<Derived, DifferentialForm::Continuous, ptScalar> {
  static Eigen::SparseMatrix<ptScalar> makeMatrix(
      const SuperSpace<Derived, ptScalar>& super_space, const int knotrepetition) {
    ...
    Eigen::SparseMatrix<ptScalar> local_matrix = projector_matrixmaker_<
        Derived, DifferentialForm::Discontinuous, ptScalar>::makeMatrix(super_space,
                                                              knotrepetition);
...
template <typename Derived, typename ptScalar>
struct projector_matrixmaker_<Derived, DifferentialForm::DivConforming, ptScalar> {
  static Eigen::SparseMatrix<ptScalar> makeMatrix(
      const SuperSpace<Derived, ptScalar>& super_space, const int knotrepetition) {
    ...
    for (int k = 0; k < size1; ++k) {
      trips.push_back(
          Eigen::Triplet<ptScalar>(info1.rows[k], info1.cols[k], info1.vals[k]));
    }
    for (int k = 0; k < size2; ++k) {
      trips.push_back(Eigen::Triplet<ptScalar>(info2.rows[k] + info1.num_dc_dof,
                                             info2.cols[k] + info1.num_c_dof,
                                             info2.vals[k]));
    }
    Eigen::SparseMatrix<ptScalar> local_matrix(
...
template <typename Derived, typename ptScalar>
struct projector_matrixmaker_<Derived, DifferentialForm::Discontinuous, ptScalar> {
  static Eigen::SparseMatrix<ptScalar> makeMatrix(
      const SuperSpace<Derived, ptScalar>& super_space, const int knotrepetition) {
    ...
    for (int k = 0; k < size; ++k) {
      trips.push_back(
          Eigen::Triplet<ptScalar>(info.rows[k], info.cols[k], info.vals[k]));
    }
    Eigen::SparseMatrix<ptScalar> local_matrix(info.num_dc_dof, info.num_c_dof);
    local_matrix.setFromTriplets(trips.begin(), trips.end());
